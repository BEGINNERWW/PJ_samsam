...
public class Fdoc_Controller {
	...
	public String FdocInsert(FdocVO vo,HttpSession session) throws Exception {
		Pattern pattern  =  Pattern.compile("<img[^>]*src=[\"']?([^>\"']+)[\"']?[^>]*>");
		String content = vo.getDoc_content();
        ...
        //mf.transferTo(new File(uploadPath+storedFileName)); // 占쏙옙占쏙옙처占쏙옙 占쏙옙占� 占십울옙占쏙옙.
        ...
        vo.setDoc_thumbnail(fdoc_ThumbNail);
        vo.setDoc_email((String)session.getAttribute("email"));
		vo.setDoc_nick((String)session.getAttribute("nick"));
	...
	private void makeThumbnail(String uploadpath,String filePath, String fileName, String fileExt) throws Exception {
		...
		if(nh > oh) { 
			nw = (oh * dw) / dh;
			nh = oh; 
		} // 占쏙옙占쏙옙 크占쏙옙占� 占쏙옙占쏙옙占싱뱄옙占쏙옙占쏙옙 占쏙옙占쏘데占쏙옙占쏙옙 crop 占쌌니댐옙. 
	...
	public String fdocView(@RequestParam(value="doc_no", required=true) int doc_no,@RequestParam(value="page", required=false,defaultValue="1")int page, Model model) {
		FdocVO vo = FdocService.getView(doc_no);
	...
	private List<FdocReflyVO> mCommentServiceList(@RequestParam int doc_no) throws Exception{
		
		List<FdocReflyVO> comment_list = FdocService.commentListService(doc_no);
	...
	*/
	...
	private int warningInsert(Myfree_doc_confirmVO vo) throws Exception{
		int res = FdocService.payInsertService(vo);
		
			FdocService.codeUpdate(vo);
	...
	public String fdoc_update(@RequestParam(value="doc_no", required=true) int doc_no,Model model) {
		FdocVO vo = FdocService.getView(doc_no);
		model.addAttribute("vo",vo);
	...
	public String fdoc_updateinsert(FdocVO vo,Model model) throws Exception {
		...
		return "redirect:/fdocdetail.bo?doc_no="+vo.getDoc_no();
	}
	
	
	@RequestMapping("/auth_fdoc.bo")
	public String getConfirmlist(Model model, @RequestParam(value="page", required=false, 
	defaultValue="1") int page,HttpSession session) {
		int limit=10;
		
		int listcount=FdocService.getConfirmCount();
		...
		hashmap.put("startrow", startrow);
		hashmap.put("endrow",endrow);
		List<ConfirmVO> confirmlist = FdocService.getConfirmList(hashmap);
		//占쏙옙 占쏙옙占쏙옙占쏙옙 占쏙옙
   		int maxpage=(int)((double)listcount/limit+0.95); //0.95占쏙옙 占쏙옙占쌔쇽옙 占시몌옙 처占쏙옙
   		//占쏙옙占쏙옙 占쏙옙占쏙옙占쏙옙占쏙옙 占쏙옙占쏙옙占쏙옙 占쏙옙占쏙옙 占쏙옙占쏙옙占쏙옙 占쏙옙(1, 11, 21 占쏙옙...)
   		int startpage = (((int) ((double)page / 10 + 0.9)) - 1) * 10 + 1;
		...
		return "js/confirm_list";
	}
	
	@RequestMapping("/fadocdetail.bo")
	public String fadocView(@RequestParam(value="doc_no", required=true) int doc_no,@RequestParam(value="page", required=false,defaultValue="1")int page, Model model) {
		ConfirmVO vo = FdocService.getConfirmView(doc_no);
		model.addAttribute("vo",vo);
		model.addAttribute("page",page);
		return "js/confirm_view";
	}
