...
<header id = "header">
	...
	<div class= "search-wrapper">
      ...
      <button class="search-box btn" type="button"><i class="fas fa-search"></i></button>
			...
			<%} %>
			&nbsp;<%=vo.getFdoc_big() %></span>
			<span class="kind_of option"><i class="fas fa-tags"></i>&nbsp;<%=vo.getFdoc_kindof() %></span>
			<span class="local" class="option"><i class="fas fa-map-marker-alt"></i>&nbsp;<%=vo.getFdoc_loc() %></span>
			</div>
			<div class="option-info">
			<span class="expiry option"><i class="fas fa-clipboard-list" title="책임기간"></i>&nbsp;<%=vo.getFdoc_expiry() %>개월</span>
			<span class="price option"><i class="fas fa-won-sign" title="책임분양비"></i>&nbsp;<%=form.format(vo.getFdoc_price()) %>원</span>
			</div>
			<div class="wrap-info">
			...
			<span><a href="#" onclick="warning_ori(<%=vo.getFdoc_no()%>,'<%=vo.getFdoc_nick()%>','<%=vo.getFdoc_subject()%>');">신고하기</a></span>
			<%}else{ %>
			<span>수정</span><span>삭제</span>
			<%}} %>
			</div>
			<div class="">
			책임 분양 안내사항&nbsp;&nbsp;<input type="checkbox" id="fdoc_agree" value="Y"><span id="agree_check"><font size="2" color="#737373">안내사항과 분양코드를 입력하세요</font></span>
    		 
    		 <div class="agreement"><textarea cols="100" rows="10" readonly style="background-color:#f3f5f7;">
    	  ...
    	  어쩌구 저쩌구어쩌구 저쩌구어쩌구 저쩌구어쩌구 저쩌구
			...
			<div><span id="code_check"><font size="2" color="#737373">분양코드는 분양자와 연락하여 받으세요</font></span>
			<span>분양코드 : <input type="text" id="fdoc_code" size="8" onblur="fdoc_check();"><input type="button" id="pay_button" disabled value="결제하기" onclick="pay();"></span>
			</div>
			</div>
